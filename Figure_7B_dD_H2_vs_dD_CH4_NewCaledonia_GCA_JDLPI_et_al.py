...
dfliteraturedata= pd.read_excel(r"Literaturedata_landserpentinisation.xlsx", index_col= "Country")   
dflitdata_volcanic= pd.read_excel(r"Literaturedata&NC_Izerumug25.xlsx", sheet_name="VolcanicContexts", index_col="IDss")
...
thisstudy= ax1.scatter(dfdataNC["dD_H2"], dfdataNC["dD_CH4"], marker= "D", s=70, color= "forestgreen", 
                       label= "Ophiolitic springs" , zorder= 5, edgecolor= "white", linewidth= 0.3)

#Plotting_ax1_Literature
# ax1.scatter(dflitdata["dD_H2"], dflitdata["dD_CH4"], marker="o", s=35, color="grey", label= "Ophiolites (Literature)", zorder= 3)

# Plotting_volcanic settings (Leila et al., 2021)
ax1.scatter(dflitdata_volcanic["dD_H2"], dflitdata_volcanic["dD_CH4"], marker= "o", s= 35, color= "violet", label= "Cont. volcanic" , zorder= 3, alpha=1)
...
# ax1.legend(handles1 + handles2, labels1 + labels2, loc='lower right', ncols= 1)
...
ax1.annotate("B", xy=(-780, -10), xytext=(0, 0), ha='center', va='top', textcoords='offset points', fontsize= 20, fontweight= "bold", zorder=6)
             

plt.savefig('Figure_7B_dD_H2_vs_dD_CH4_NewCaledonia_GCA_JDLPI_et_al.svg', format='svg', bbox_inches='tight')
plt.savefig('Figure_7B_dD_H2_vs_dD_CH4_NewCaledonia_GCA_JDLPI_et_al.tiff', format='tiff', bbox_inches='tight')
