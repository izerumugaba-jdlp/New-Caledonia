...
dflitfischer= pd.read_excel(r"Litdata_N2_Fischer2002.xlsx")
...
# Error bars _ d15N
plt.errorbar(dfendmembers_f.loc[:,"d15N_m"], dfendmembers_f.loc[:,"N2/He"], 
             xerr=dfendmembers_f.loc[:,"err_d15N_m"], fmt="o", color="red", capsize=4, 
             zorder= 0, alpha= 0.3)
...
ax.scatter(ophiolitic["d15N_m"],((ophiolitic["N2"]/100)/(ophiolitic["4He"]+ophiolitic["3He"])), marker= "D", s=45, color= "forestgreen", label= "Ophiolitic springs", zorder= 5)
ax.scatter(basement["d15N_m"],((basement["N2"]/100)/(basement["4He"]+basement["3He"])), marker= "D", s=45, color= "brown", label= "Basement springs", zorder= 5)


# Plotting literature data Fischer et al., 2022
ax.scatter(dflitfischer["d15N"], dflitfischer["N2/He"], s=35, edgecolors="black", facecolors= "none", marker="o", label="C. American subd.", zorder= 4)
"""
...
"""
ax.scatter(dflitdata["d_15N"], (dflitdata["N2"]/100)/(dflitdata["4He"]+dflitdata["3He"]),
    marker="o", s=35, color="grey", label="Ophiolites (literature)", zorder= 4)
"""
...
plt.xlim(-8, 8)
plt.ylim(10**1, 10**6)
plt.legend(loc= "lower right", ncol= 2, fontsize= 9, columnspacing=0.3, labelspacing=0.6, handletextpad=0.01)
# plt.xscale("log")
...
for i in dfendmembers.index:
    if i==0:
        ...
        ax.annotate((dfendmembers.loc[i,"Label"]), xy=(dfendmembers.loc[i,"d15N_m"], dfendmembers.loc[i,"N2/He"]), 
                        fontsize= 11, xytext=(-5, 7), textcoords='offset points')
    elif i==3:
        ax.annotate((dfendmembers.loc[i,"Label"]), xy=(dfendmembers.loc[i,"d15N_m"], dfendmembers.loc[i,"N2/He"]), 
                        fontsize= 11, xytext=(-10, -15), textcoords='offset points')
    elif i==1:
        ax.annotate((dfendmembers.loc[i,"Label"]), xy=(dfendmembers.loc[i,"d15N_m"], dfendmembers.loc[i,"N2/He"]), 
                        fontsize= 11, xytext=(-17, 7), textcoords='offset points')
...
plt.savefig("Figure_11_A_d15N_vs_N2He_NewCaledonia_GCA_JDLPI_et_al.svg", format= "svg", bbox_inches='tight')
plt.savefig("Figure_11_A_d15N_vs_N2He_NewCaledonia_GCA_JDLPI_et_al.tiff", format= "tiff", bbox_inches='tight')
