...
dfdata= pd.read_excel(r"Test_Hisotopefractionation.xlsx", sheet_name= "ThisStudy_NC", index_col= "IDs")

dflitdata= pd.read_excel(r"Literaturedata.xlsx", sheet_name="Serpent.Contexts_plot", 
                         index_col="IDss")
...
for Type, marker, color, label, zorder in [
        ("Ophiolitic", "D", "forestgreen", "Ophiolitic", 5),
        ("Basement", "D", "brown", "Basement", 6)]:
    df_Type= dfdata[dfdata["Type"] == Type]
    plt.scatter(df_Type["dD_CH4"], df_Type["d13C_CH4"], marker= marker, s=75, color= color, label= label, zorder= zorder, edgecolor= "white", linewidth= 0.3)
...
# ax.scatter(dflitdata["dD_CH4"], dflitdata["d13C_CH4"], marker="o", s=35, color= "grey", alpha= 1, zorder= 4)
...
plt.savefig('Figure_7A_dD_CH4vsd13C_CH4_NewCaledonia_GCA_JDLPI_et_al.svg', format='svg', bbox_inches='tight')
plt.savefig('Figure_7A_dD_CH4vsd13C_CH4_NewCaledonia_GCA_JDLPI_et_al.tiff', format='tiff', bbox_inches='tight')
