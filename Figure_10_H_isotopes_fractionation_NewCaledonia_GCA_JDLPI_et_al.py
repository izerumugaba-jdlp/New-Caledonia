...
dfdata= pd.read_excel(r"Test_Hisotopefractionation.xlsx", index_col=("Samples"))
...
plt.savefig('Figure_10A_H_isotopes_fract_alpha_NewCaledonia_GCA_JDLPI_et_al.svg', format='svg', bbox_inches='tight')
plt.savefig('Figure_10A_H_isotopes_fract_alpha_NewCaledonia_GCA_JDLPI_et_al.tiff', format='tiff', bbox_inches='tight')
...
for site, marker, markersize, color, label, zorder, alpha in [
        ("Larderello", "o", 50, "black", "Larderello", 3, 1),
        ("Oman", "o", 50, "grey", "Ophiolites (literature)", 3, 1),
        ("UAE", "o", 50, "aqua", "UAE", 1, 1),         #aqua
        ("Japan", "o", 50, "darkorchid", "Japan", 3, 1),     #darkorchid
        ("Philippines", "o", 50, "orange", "Philippines", 3, 1),    #orange
        ("Turkey", "o", 50, "lightcoral", "Turkey", 3, 1),            #lightcoral
        ("NC", "D", 70, "forestgreen", "Ophiolitic (NC)", 4, 1)]:
    df_site= dfdata[dfdata["Site"] == site]
    datapoints= plt.scatter(df_site["epsilon_H2_H2O"], df_site["epsilon_CH4_H2O"], marker= marker, s= markersize, color= color, label= label, alpha= alpha)
...
htos= ax.add_patch(pltp.Ellipse((-480, -110), 250, 120, angle=0, linewidth=1, linestyle="--", edgecolor='none', facecolor='steelblue', alpha= 0.7, zorder=0, label="Hight T. Oceanic")) #HightempOcSet


# LINES ESPILON(CH4-H2)_ CH4-H2 FRACTION AT DIFFERENT TEMPERATURES (SUDA ET AL., 2014)
# reference Line is the 100°C line which passes thru P1(-800, 0) and P2(-1000, -200). 
# calculated slope= 1; equation of line is y = x + 800. change intercept to plot parallel lines at other temps.
# the correlated x and y values are: x_eps_H2_H20= e.g. np.linspace(-1600, 0, 100) ; y_eps_CH4_H20= e.g. np.linspace(-900, 0, 100) 
x_eps_CH4_H2= np.linspace(-1600, 0, 100)
y_eps_CH4_H2_100 = x_eps_CH4_H2 + 800
y_eps_CH4_H2_50 = x_eps_CH4_H2 + 980
y_eps_CH4_H2_eq = x_eps_CH4_H2

plt.plot(x_eps_CH4_H2, y_eps_CH4_H2_100, "k--", zorder= 0, alpha= 0.8, label= "ε(CH4-H2)")  # 100
plt.plot(x_eps_CH4_H2, y_eps_CH4_H2_50, "k--", zorder= 0, alpha= 0.8)  # 50
plt.plot(x_eps_CH4_H2, y_eps_CH4_H2_eq, "k--", zorder= 0, alpha= 0.8)  # eq

plt.annotate("ε($\mathbf{CH_4-H_2}$)= 0‰", xy=(-200, -50), xytext=(0, 0), ha='center', va='top', 
             textcoords='offset points', fontsize= 12, fontweight='bold', rotation= 50)
...
plt.annotate("B", xy=(-1555, -9), xytext=(0, 0), ha='center', va='top', 
             textcoords='offset points', fontsize= 20, fontweight= "bold", zorder=6)

plt.savefig('Figure_10B_H_isotopes_fract_epsilon_NewCaledonia_GCA_JDLPI_et_al.svg', format='svg', bbox_inches='tight')
plt.savefig('Figure_10B_H_isotopes_fract_epsilon_NewCaledonia_GCA_JDLPI_et_al.tiff', format='tiff', bbox_inches='tight')
